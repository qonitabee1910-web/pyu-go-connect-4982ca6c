            ...
            <div key={idx} className="border rounded-lg p-3 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => movePoint(idx, -1)} disabled={idx === 0}><ArrowUp className="w-3 h-3" /></Button>
                  <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => movePoint(idx, 1)} disabled={idx === points.length - 1}><ArrowDown className="w-3 h-3" /></Button>
                  <Button size="icon" variant="ghost" className="h-6 w-6 text-destructive" onClick={() => removePoint(idx)}><Trash2 className="w-3 h-3" /></Button>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div><Label className="text-xs">Nama</Label><Input className="h-8 text-xs" value={p.name} onChange={(e) => updatePoint(idx, "name", e.target.value)} placeholder="Hermes Palace" /></div>
                <div><Label className="text-xs">Waktu</Label><Input className="h-8 text-xs" value={p.departure_time} onChange={(e) => updatePoint(idx, "departure_time", e.target.value)} placeholder="06.00" /></div>
                <div><Label className="text-xs">Jarak (Mtr)</Label><Input className="h-8 text-xs" type="number" value={p.distance_meters} onChange={(e) => updatePoint(idx, "distance_meters", Number(e.target.value))} /></div>
                <div><Label className="text-xs">Tarif (Rp)</Label><Input className="h-8 text-xs" type="number" value={p.fare} onChange={(e) => updatePoint(idx, "fare", Number(e.target.value))} /></div>
              </div>
            </div>
          ))}
        </div>
...
export default function RayonsTab() {
  const qc = useQueryClient();
  ...
  return (
    <div>
      <div className="flex justify-end mb-3">
        <Button size="sm" className="gradient-primary text-primary-foreground" onClick={() => { setEditRayon(null); setDialogOpen(true); }}>
              ...
              {expandedId === r.id && (
                <CardContent className="space-y-2">
                  {r.pickup_points.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead><tr className="text-muted-foreground border-b"><th className="text-left py-1 pr-2">Order</th><th className="text-left py-1 pr-2">Nama</th><th className="text-left py-1 pr-2">Waktu</th><th className="text-right py-1 pr-2">Jarak</th><th className="text-right py-1">Tarif</th></tr></thead>
                        <tbody>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button size="sm" variant="outline" onClick={() => { setEditRayon(r); setDialogOpen(true); }}><Edit className="w-3 h-3 mr-1" />Edit</Button>
                    <Button size="sm" variant="outline" className="text-destructive" onClick={() => deleteMutation.mutate(r.id)}><Trash2 className="w-3 h-3 mr-1" />Hapus</Button>
                  </div>
                </CardContent>
              )}
            </Card>
